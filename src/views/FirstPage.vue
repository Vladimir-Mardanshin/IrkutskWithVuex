<template>
    <h1 style="font-family: Comic Sans MS;">Достопримечательности</h1>
    <v-container>
        <v-row class="list__cafes-content">
            <MapPlace v-for="place in allPlacesIrk" :key="place.id" :place="place"></MapPlace>
        </v-row>
    </v-container>
    ...
    <v-overlay v-model="overlay" class="align-center justify-center">
        ...
        <div class="pumpum">
            <h2 style="margin-bottom: 10px;">Новая достопримечательность:</h2>
...
export default {
    ...
    computed: mapGetters(["allPlacesIrk"]),
    data() {
        return {
            overlay: false,
            title: '',
    ...
    methods: {
        ...mapMutations(['createPlacesIrk']),
        submit() {
            if ((this.title === "") || (this.body === "") || (this.url === "")) {
                alert('Заполните необходимые поля!');
            } else {
                this.createPlacesIrk({
                    id: Date.now(),
                    name: this.title,
                    info: this.body,
                    photo: this.url
